...
class Import {
  ...
  importFromFile(conversations_path, messages_path = "") {
    ...
    // Convert JSONL file first
    if (conversations_path.endsWith(".jsonl")) {
      let _jsonFilePath = conversations_path.replace(".jsonl", ".json");
      
      return new Promise((resolve, reject) => {
        let _readStream = fs.createReadStream(conversations_path);
        let _writeStream = fs.createWriteStream(_jsonFilePath);
        ...
        _writeStream.on("finish", () => {
          resolve(this.importFromFile(_jsonFilePath));
        });
