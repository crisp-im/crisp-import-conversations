...
var { jsonlToJson }          = require("./helpers/jsonl");
...
var RETRY_DELAY            = 60; // 60 seconds
var IMPORT_MARKER_OFFSET   = 60; // 60 seconds
var SECOND_TO_MILLISECOND  = 1000;
var HTTP_QUOTA_EXCEEDED    = 423;
 ...
 * Import
 * @class
 * @classdesc Handles the import of conversations from various sources to Crisp
 */
class Import {
  /**
   * Constructor
  ...
  constructor(config, options={}) {
    ...
    this.planLimits = PLAN_LIMITS[
      (this.config.websitePlan || "plus").toLowerCase()
    ];

    this.backPressure = BACK_PRESSURE;

    if (this.planLimits?.conversation) {
      // Compute back pressure based on plan
      this.backPressure = Math.ceil(
        (this.planLimits.conversation.ttl * SECOND_TO_MILLISECOND) /
          this.planLimits.conversation.threshold
      );
    }

    // Regex patterns
    this.inlinedImageRegex = /src="(data:image\/[^;]+;base64[^"]+)"/gm;
    this.newlineRegex      = /\n/g;
    this.carriageRegex     = /\r/g;
  ...
  importFromFile(conversations_path, messages_path="") {
    ...
    // Convert JSONL file?
    if (conversations_path.endsWith(".jsonl")) {
      return Promise.resolve()
        .then(() => {
          // Convert to JSON
          return jsonlToJson(
    ...
    return new Promise((resolve, reject) => {
      let _pipeline = fs.createReadStream(conversations_path)
        .on("error", () => {
          return reject(
            new Error("Couldn't open your file, is the path valid?")
          );
        })
        ...
        .on("error", () => {
          return reject(new Error(
            ...
            (
              (this.options.adapter === "zendesk")
                ? "When importing from Zendesk, you might need to " +
                    "replace the `json` extension with `jsonl`\n"
                : ""
            ) +
  ...
  importConversation(conversation) {
    ...
    return Promise.resolve()
      ...
      .then((conversation) => {
        ...
        return this.__callWithRetry(() =>
          this.client.website.createNewConversation(this.config.websiteId)
        );
      })
      .then((result) => {
        ...
        _meta.email = (
          _conversation.user?.email || this.config.defaultUserEmail
        );
        ...
        if (_conversation.user?.name) {
          _meta.nickname = (
            _conversation.user.name || this.config.defaultUserNickname
          );
        }
      ...
      .then(() => {
        if (conversation.participants?.length > 0) {
          ...
          if (_limit && _skipped > 0) {
            console.warn(
              `⚠️  Skipping ${_skipped} ` +
                `participant${(_skipped > 1) ? "s" : ""} ` +
                `because plan is ${this.planLimits.name}`
            );
          ...
          if (_participants.length) {
            return this.__callWithRetry(() =>
              ...
              .catch((error) => {
                console.warn(
                  `⚠️  Couldn't create ${_participants.length} participants `
                    `because:\n`, error
                );
      ...
      .then(() => {
        let _sortedMessages = _conversation.messages?.sort((first, second) => {
        ...
        return this.__sendMessage({
          message : {
            ...
            from       : "operator",
            date       : (
              _firstMessage.date - IMPORT_MARKER_OFFSET * SECOND_TO_MILLISECOND
            ),
      ...
      .then(() => {
        ...
        return this.__callWithRetry(() =>
          this.client.website.markMessagesReadInConversation(
            this.config.websiteId,
            _sessionId,
      ...
      .then(() => {
        return this.__sendMessage({
          message : {
            ...
            from       : "operator",
            date       : (
              _lastMessage.date + IMPORT_MARKER_OFFSET * SECOND_TO_MILLISECOND
            ),
      ...
      .then(() => {
        return this.__callWithRetry(() =>
          this.client.website.changeConversationState(
            this.config.websiteId,
            _sessionId,
      ...
      .catch((error) => {
        ...
        console.error(
          `⚠️  Error creating conversation${error ? ": " : ""}`,
          error ? error : ""
        );
  ...
  /**
   * Calls a function and retries if needed
   * @private
   * @param  {function} fn
   * @return {object}   Promise object
   */
  __callWithRetry(fn) {
    return fn()
      .catch((error) => {
        ...
        if (
          _code === HTTP_QUOTA_EXCEEDED ||
            _message === "quota_limit_exceeded"
        ) {
          console.warn(
            `⚠️  Quota limit exceeded (423). ` +
              `Waiting ${RETRY_DELAY}s before retrying...`
          );

          return temporize(RETRY_DELAY * SECOND_TO_MILLISECOND)
            .then(() => fn());
        }
  ...
  __sendMessage({ message, user = {}, conversation = {} }) {
    let _message_params;
    ...
    return Promise.resolve()
      .then(() => {
        ...
        if (Object.keys(_user).length === 0) {
          if (_from === "user") {
            // Use conversation user
            _user = conversation.user;

            // Enforce default nickname
            _user.name = _user.name || this.config.defaultUserNickname;
          }
          ...
          if (_from === "operator") {
            ...
            _user = {
              name : (
                this.config.defaultOperatorNickname ||
                  this.config.name ||
                  "Import"
              )
            };
        ...
        if (message.original?.content) {
          ...
          message.original.content = message.original.content
            .replace(this.inlinedImageRegex, (match) => {
      ...
      .then((message_params) => {
        ...
        // Check empty content?
        if (typeof _message_params.content === "string") {
          let _stripped_content = _message_params.content || "";

          _stripped_content = _stripped_content.replace(
            this.newlineRegex, " "
          );
          ...
          if (_stripped_content.trim() === "") {
            console.warn(
              "⚠️  Skipping message because content is empty"
            );
        ...
        if (!error) {
          console.error(
            "⚠️  Error applying the adapter: " +
              `Please make sure the '${this.options.adapter}' adapter exists ` +
              "and provides a valid `adapt_conversation()` method"
          );
