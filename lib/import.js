...
class Import {
  ...
  constructor(config, options = {}) {
    ...
    if (this.planLimits?.conversation) {
      ...
      this.backPressure = Math.ceil(
        (this.planLimits.conversation.ttl * 1000) / this.planLimits.conversation.threshold
      );
    }
  ...
  /**
   * Calls a function and retries after 60s if a 423 quota error is received
   * @private
  ...
  __callWithRetry(fn) {
    return fn().catch((error) => {
      ...
      if (code === 423 || message === "quota_limit_exceeded") {
        const waitSeconds = 60;

        console.warn(`⚠️  Quota limit exceeded (423). Waiting ${waitSeconds}s before retrying...`);
          ...
          return reject(new Error(
            ...
            (
              this.options.adapter === "zendesk" ?
                "When importing from Zendesk, you might need to replace the `json` extension with `jsonl`\n" :
                ""
            ) +
            `The format must be:
            [
  ...
  importConversation(conversation) {
    ...
    return Promise.resolve()
      ...
      .then((result) => {
        ...
        _meta.email = (_conversation.user || {}).email || this.config.defaultUserEmail;
        ...
        if ((_conversation.user || {}).name) {
          _meta.nickname = _conversation.user.name || this.config.defaultUserNickname;
        }
    ...
    return Promise.resolve()
      .then(() => {
        ...
        if (Object.keys(_user).length === 0) {
          ...
          if (_from === "operator") {
            // Enforce default nickname
            _user = {
              name : this.config.defaultOperatorNickname || this.config.name || "Import"
            };
          }
        }
      ...
      .then((message_params) => {
        ...
        if (typeof _message_params.content === "string") {
          ...
          _stripped_content = _stripped_content.replace(/\r/g, " ");
    ...
    return Promise.resolve()
      ...
      .then((adapter) => {
        ...
        if (typeof this.adapter.adapt_conversation === "function") {
          return this.adapter.adapt_conversation(conversation, this.options);
        }
