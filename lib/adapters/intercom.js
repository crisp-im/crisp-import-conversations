...
var adapt_conversation = (conv, options = {}) => {
  let GCS_BASE_URL = options.gcsBaseUrl;

  if (!GCS_BASE_URL) {
    throw new Error("gcsBaseUrl is required in options");
  }

  // Remove trailing slash to avoid double slashes in constructed URLs
  GCS_BASE_URL = GCS_BASE_URL.replace(/\/+$/, '');
  ...
  if (conv.source && conv.source.body) {
    ...
    (conv.source.attachments || []).forEach((attachment, attachmentIndex) => {
      if (!attachment.name || !conv.id || !conv.source.id) {
        console.warn(`⚠️  Skipping attachment: missing name, conv.id or conv.source.id`);
        return;
      }

      const gcsUrl = `${GCS_BASE_URL}/attachments/${encodeURIComponent(conv.id)}/${encodeURIComponent(conv.source.id)}/${encodeURIComponent(attachment.name)}`;
  ...
  for (const part of parts) {
    ...
    (part.attachments || []).forEach((attachment, attachmentIndex) => {
      if (!attachment.name || !conv.id || !part.id) {
        console.warn(`⚠️  Skipping attachment: missing name, conv.id or part.id`);
        return;
      }

      const gcsUrl = `${GCS_BASE_URL}/attachments/${encodeURIComponent(conv.id)}/${encodeURIComponent(part.id)}/${encodeURIComponent(attachment.name)}`;
  ...
  // Filter out messages with no text, note, or file content
  const filteredMessages = messages.filter(m => {
    if (m.file) return true;
    if (m.note && m.note.trim() !== "") return true;
