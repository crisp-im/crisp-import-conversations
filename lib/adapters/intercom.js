...
var processAttachments = (attachments, conversationId, partId, author, baseDate, authorName) => {
  let _getAttachmentLocalPath = (attachment) => {
    return path.join("./res/attachments", String(conversationId), String(partId), attachment.name);
  };

  return attachments
    .filter((attachment) => {
      if (!attachment.name || !conversationId || !partId) {
        console.warn(`⚠️  Skipping attachment: missing name, conversationId or partId`);
  ...
  if (conversation.source?.body) {
    ...
    if (conversation.source.attachments?.length > 0) {
      let _attachmentMessages = processAttachments(
        ...
        conversation.source.author,
        (conversation.created_at || 0) * 1000,
